...
func main() {
	...
	if debug {
		...
		global.ServerConf.Port = port
	}

	// 4、grpc注册
	server := grpc.NewServer()
	proto.RegisterUserServer(server, &handler.UserServer{})
	listen, err := net.Listen("tcp", fmt.Sprintf("%s:%d", global.ServerConf.Host, global.ServerConf.Port))
	if err != nil {
		panic(err)
	}
	...
	// 7、 服务发现
	go func() {
		zap.S().Infof("gRPC 服务器成功启动在 %s:%d", global.ServerConf.Host, global.ServerConf.Port)
